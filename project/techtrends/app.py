...
def post(post_id):
    ...
    if post is None:
      app.logger.info(f'Article with post_id "{post_id}" not found!')
      return render_template('404.html'), 404
    else:
      article_name = post['title']
      app.logger.info(f'Article "{article_name}" retrieved!')
...
def metrics():
    ...
    post_count = result[0] if result is not None else None
    connection.close()
    response = app.response_class(
